...
SOURCES		:=	$(UNIVCORE) source source/gui source/overlays source/screens source/screens/local source/utils
DATA		:=	data
INCLUDES	:=	$(UNIVCORE) include include/gui include/overlays include/screens include/screens/local include/utils
GRAPHICS	:=	assets/gfx
ROMFS		:=	romfs
GFXBUILD	:=	$(ROMFS)/gfx
APP_AUTHOR	:=	SuperSaiyajinStackZ
APP_DESCRIPTION := Connect Four clone for Nintendo 3DS.
ICON		:=	app/icon.png
...
$(OUTPUT).cia	:	$(OUTPUT).elf $(OUTPUT).smdh
	...
	$(MAKEROM) -f cia -target t -exefslogo -o "../3DVier.cia" -elf "../3DVier.elf" -rsf "../app/build-cia.rsf" -banner "../app/banner.bin" -icon "../app/icon.bin" -logo "../app/logo.bcma.lz" -DAPP_ROMFS="$(TOPDIR)/$(ROMFS)" -major $(VERSION_MAJOR) -minor $(VERSION_MINOR) -micro $(VERSION_MICRO) -DAPP_VERSION_MAJOR="$(VERSION_MAJOR)"
