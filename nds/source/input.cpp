...
int kbDirections = 0, kbGroupAmount = 0, kbLayout = 3, kbXPos = 0;
...
void processInputABC(u16 held, unsigned maxLength) {
	...
	while(keysHeld() & (KEY_UP | KEY_DOWN | KEY_LEFT | KEY_RIGHT)) {
		...
		if (direction != -1) {
			...
			std::u16string *character;
				if (kbDirections) character = &(kbGroupAmount ? keysDPadABC4 : keysDPadABC3)[direction + ((keysHeld() & KEY_L) ? 4 : 0)];
				else character = (keysHeld() & KEY_L) ? &keysDPadABCSymbols[direction] : (kbGroupAmount ? &keysDPadABC4[direction] : &keysDPadABC3[direction]);
			std::pair<int, int> *pos = (kbDirections ? &keysDPad4[direction] : &keysDPad8[direction]);
...
std::string Input::getLine(std::string Text, unsigned maxLength, std::string currentString, bool showPointer, bool redrawScreen) {
	...
	while(1) {
		...
		if (held & KEY_START || enter) {
			int prevLayout = kbLayout, prevXPos = kbXPos;
			kbLayout = loadedLayout;
			if (loadedLayout < 3) kbXPos = xPos;
			break;
